...
const Carrinho = () => {
    ...
    const [opcaoFrete, setOpcaoFrete] = useState('lagoa'); // Opções: 'lagoa', 'fora' ('externo' renomeado), 'rural'
    const [valorFrete, setValorFrete] = useState(0);
    const [cidadeFreteExterno, setCidadeFreteExterno] = useState('');
    const valorFreteRural = 15;

    // Valores de frete fixos por cidade (para esta versão)
    ...
    const calcularTotalCarrinho = () => {
        return cart.reduce((total, item) => total + calcularSubtotalItem(item), 0) + valorFrete;
    };
    ...
    const handleFinalizarPedido = () => {
        if (cart.length > 0) {
            navigate('/cliente/cadastro', { state: { cart, valorFrete } }); // Passa o valor do frete
        } else {
    ...
    const handleOpcaoFreteChange = (event) => {
        ...
        setOpcaoFrete(opcao);
        setValorFrete(0);
        setCidadeFreteExterno(''); // Limpa a cidade ao mudar a opção
    ...
    const handleCidadeFreteExternoChange = (event) => {
        ...
        if (opcaoFrete === 'fora' && valoresFreteExterno[cidadeSelecionada]) {
            ...
            setValorFrete(0); // Resetar o frete se a cidade não estiver na lista ou a opção de frete não for 'fora'
        }
    };
    ...
    if (cart.length === 0) {
        return (
            <div className="carrinho-container carrinho-vazio">
                ...
                <p>Adicione produtos na página de vendas para começar.</p>
                <button onClick={handleContinuarComprando} className="continuar-comprando-button">
                    Continuar Comprando
                </button>
            </div>
        );
    ...
    return (
        <div className="carrinho-container">
            ...
            <ul className="carrinho-lista">
                {cart.map((item) => (
                    <li key={item.id} className="carrinho-item">
                        ...
                        <button onClick={() => handleRemoverItem(item)} className="remover-item-button">
                            Remover
                        </button>
                    </li>
                ))}
            ...
            <div className="frete-container">
                ...
                <div className="opcao-frete-externo">
                    <input
                        type="radio"
                        id="fora"
                        value="fora"
